...
//	typedef Eigen::UmfPackLU<EMatrix> SolverType;
	...
	MatrixInterface(EMatrix const &mat)
		: num_request(0),num_converged(0),compute_vecs(1),verbose_diagonalize(0), UseShiftAndInvert(0), Eigenvalues(NULL), Eigenvectors(NULL), LUdecomposed(false), A(mat)
	{
	};
...
void MatrixInterface<T>::Diagonalize()
{
	...
	{
		std::cout << "Shift and Invert mode is on. First perform an LU decomposition... ";
		LUdecomp();
	...
	else					num_request	= std::min((int)A.rows()-1, num_request);
        std::cout <<"num request: " << num_request << std::endl << std::flush;
