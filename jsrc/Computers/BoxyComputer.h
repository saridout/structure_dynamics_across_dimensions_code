...
//		This version of the class adjusts the box size to reach a target pressure.
//
//Variables
...
template <int Dim>
class CBoxyComputer : public CBaseComputer<Dim>
{
private:
...
public:
	...
	CBoxyComputer();
	CBoxyComputer(CStaticState<Dim> &_State, dbl p);
	CBoxyComputer(CStaticState<Dim> &_State, dbl p, dbl t_r);
	CBoxyComputer(const CBoxyComputer<Dim> &_Copy);
...
//Needed for minimization routines
	...
	//Shear steps added by S.A.R. (see files in "Protocols")
	void AffineShear(dmat const &strain_tensor);
	void ShearWithNonAffine(dmat const &strain_tensor);
};
...
template <int Dim>
CBoxyComputer<Dim>::CBoxyComputer() : FixDof(false), UseGrid(true)
{
}


template <int Dim>
CBoxyComputer<Dim>::CBoxyComputer(CStaticState<Dim> &_State, dbl p) : State(_State), P_targ(p), Grid(&State), FixDof(false), UseGrid(true)
{
//	Grid.Allocate();
	dvec MaxDistance;
...
template <int Dim>
CBoxyComputer<Dim>::CBoxyComputer(CStaticState<Dim> &_State, dbl p, dbl t) : State(_State), P_targ(p), Grid(&State), FixDof(false), UseGrid(true)
{
//	Grid.Allocate();
	dvec MaxDistance;
	State.GetMaxDistance(MaxDistance);
...
template <int Dim>
CBoxyComputer<Dim>::CBoxyComputer(const CBoxyComputer<Dim> &_Copy) : State(_Copy.State), Grid(&State), FixDof(_Copy.FixDof), FixedDof(_Copy.FixedDof), UseGrid(_Copy.UseGrid)
{
//	Grid.Allocate();
...
//get and set the state
template <int Dim>
CStaticState<Dim> &CBoxyComputer<Dim>::GetState() const
{
	return State;
}

template <int Dim>
void CBoxyComputer<Dim>::SetState(CStaticState<Dim> & _State)
{
	State = _State;
	Grid.SetState(&State);
}
...
template <int Dim>
dbl CBoxyComputer<Dim>::GetPTarg() const
{
	return P_targ;
}

template <int Dim>
void CBoxyComputer<Dim>::SetPTarg(dbl p)
{
	P_targ = p;
}
	
template <int Dim>
inline dbl CBoxyComputer<Dim>::GetVolume() const
{
	return State.GetVolume();
}

template <int Dim>
inline int CBoxyComputer<Dim>::GetNdof() const
{
	return State.GetParticleNumber()*Dim;
}

template <int Dim>
inline dbl CBoxyComputer<Dim>::GetMinimizationTimeScale() const
{
...
template <int Dim>
void CBoxyComputer<Dim>::SetUseGrid(bool usegrid)
{
	UseGrid = usegrid;
}

template <int Dim>
CGrid<Dim>& CBoxyComputer<Dim>::GetGrid() 
{
	return Grid;
}
...
//Compute the bond list
template <int Dim>
void CBoxyComputer<Dim>::ComputeBondList(CBondList<Dim> &bonds)
{
	if(UseGrid)
...
template <int Dim>
void CBoxyComputer<Dim>::ComputeBondList_Grid(CBondList<Dim> &bonds)
{
...
template <int Dim>
void CBoxyComputer<Dim>::ComputeBondList_NoGrid(CBondList<Dim> &bonds) const
{
	//Make sure bonds is empty.
	bonds.RemoveAllBonds();
 ...
 */
template <int Dim>
int CBoxyComputer<Dim>::StdPrepareSystem(bool verbose)
{
 ...
 */
template <int Dim>
int CBoxyComputer<Dim>::StdPrepareSystem(vector<bool> const &fixed, bool verbose)
{
...
template <int Dim>
void CBoxyComputer<Dim>::CalculateStdData(bool CalcCijkl, bool CalcHess)
{
	CalculateStdData(Data,CalcCijkl,CalcHess);
}
template <int Dim>
void CBoxyComputer<Dim>::CalculateStdData(CStdData<Dim> &data, bool CalcCijkl, bool CalcHess)
{
...
template <int Dim>
void CBoxyComputer<Dim>::CalculateStdData_Unstressed(bool CalcCijkl, bool CalcHess)
{
...
template <int Dim>
void CBoxyComputer<Dim>::CalculateStdData_Unstressed(CStdData<Dim> &data, bool CalcCijkl, bool CalcHess)
{
...
template <int Dim>
void CBoxyComputer<Dim>::SetFixedDof(vector<bool> const &_FixedDof)
{
	FixDof = true;
	FixedDof = _FixedDof;
...
template <int Dim>
void CBoxyComputer<Dim>::UnsetFixedDof()
{
	FixDof = false;
}

template <int Dim>
void CBoxyComputer<Dim>::Evaluate(Eigen::VectorXd &grad, dbl &fx)
{
	CBondList<Dim> bonds;
	ComputeBondList(bonds);
	fx = FixDof?bonds.ComputeVolGradient(grad,P_targ,FixedDof):bonds.ComputeVolGradient(grad,P_targ);
};

template <int Dim>
bool CBoxyComputer<Dim>::Progress(Eigen::VectorXd const &grad, dbl fx, int iteration, int print_iter, dbl tol) 
{
...
template <int Dim>
bool CBoxyComputer<Dim>::ProgressSilent(Eigen::VectorXd const &grad, dbl fx, int iteration, int print_iter, dbl tol) 
{
	dbl gradNorm = grad.norm();
	dbl max_grad = max_abs_element(grad.size(), grad.data());
	bool converged = (max_grad < tol)?true:false;
...
template <int Dim>
inline void CBoxyComputer<Dim>::Move(Eigen::VectorXd const &step) 
{
...
template <int Dim>
void CBoxyComputer<Dim>::ComputeHessian(Eigen::SparseMatrix<dbl> &hess) const
{
	ComputeHessian(Bonds, hess);
}

template <int Dim>
void CBoxyComputer<Dim>::ComputeHessian(CBondList<Dim> const &bonds, Eigen::SparseMatrix<dbl> &hess) const
{
	bonds.ComputeHessian(hess);
}

template <int Dim>
void CBoxyComputer<Dim>::ComputeHessianBZ(Eigen::SparseMatrix<cdbl> &hess, Eigen::SparseMatrix<cdbl> &transformation, dvec const &k) const
{
	ComputeHessianBZ(Bonds, hess, transformation, k);
}
template <int Dim>
void CBoxyComputer<Dim>::ComputeHessianBZ(CBondList<Dim> const &bonds, Eigen::SparseMatrix<cdbl> &hess, Eigen::SparseMatrix<cdbl> &transformation, dvec const &k) const
{
...
template <int Dim>
void CBoxyComputer<Dim>::CalculateBZTransformation(Eigen::SparseMatrix<cdbl> &transformation, dvec const &k) const
{
	CalculateBZTransformation(Bonds, transformation, k);
}

template <int Dim>
void CBoxyComputer<Dim>::CalculateBZTransformation(CBondList<Dim> const &bonds, Eigen::SparseMatrix<cdbl> &transformation, dvec const &k) const
{
...
template<int Dim>
void CBoxyComputer<Dim>::AffineShear(dmat const &strain_tensor) {
	dmat box;
	State.GetBox()->GetTransformation(box);
box *= dmat::Identity()+strain_tensor;
	State.GetBox()->SetTransformation(box);
}


template<int Dim>
void CBoxyComputer<Dim>::ShearWithNonAffine(dmat const &strain_tensor) {
	MatrixInterface<dbl> D1;
	Bonds.ComputeHessian(D1.A, 1.0,1.0, 1e-12);
	D1.LUdecomp();
	int Nvar = Dim*RattlerMap.size();
	Eigen::VectorXd n_d2Udvdgamma = Eigen::VectorXd::Zero(Nvar);
	Eigen::VectorXd uNonAffine_node = Eigen::VectorXd::Zero(Nvar);
	Eigen::VectorXd DeltaR_bond = Eigen::VectorXd::Zero(Dim*(int)Bonds.GetNBonds());

	Bonds.Calculate_n_d2Udvdgamma(strain_tensor, n_d2Udvdgamma, DeltaR_bond, 1.0,1.0);

	//Solve for the non-affine displacement
	D1.solve_Mx_equals_b(uNonAffine_node, n_d2Udvdgamma);

	//now apply both shear and non-affine displacements
	dmat box;
	State.GetBox()->GetTransformation(box);
		box *= dmat::Identity()+strain_tensor;
	State.GetBox()->SetTransformation(box);
	Eigen::VectorXd full_uNA = Eigen::VectorXd::Zero(State.GetParticleNumber()*Dim);
	for (int i = 0; i < RattlerMap.size(); i++) {
		full_uNA.segment<Dim>(Dim*RattlerMap[i]) = -uNonAffine_node.segment<Dim>(Dim*i);
	}
	State.MoveParticles(full_uNA);
}
...
template <int Dim>
dbl CBoxyComputer<Dim>::ComputeEnergy()
{
	Grid.Construct();
...
template <int Dim>
void CBoxyComputer<Dim>::ComputeGradient(Eigen::VectorXd &tar)
{
	Grid.Construct();
...
template <int Dim>
void CBoxyComputer<Dim>::ComputeForce(Eigen::VectorXd &tar)
{
	ComputeGradient(tar);
	tar = -tar;
}
...
//Hessian stuff
template <int Dim>
void CBoxyComputer<Dim>::ComputeHessian(Eigen::MatrixXd &tar) 
{
	Grid.Construct();
...
//computes at q = 0
template <int Dim>
void CBoxyComputer<Dim>::ComputeDynamicalMatrix(Eigen::MatrixXd &tar)
{
	Grid.Construct();
