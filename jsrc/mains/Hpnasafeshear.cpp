...
* Apply exact nonaffine linear response before minimizing.
...
#include "Computers/BoxyComputer.h"
...
int main(int argc, char *argv[])
{
  ...
  int steps = 0;
	dbl guess_phi = 0.95;
	int in;
        while((in = getopt(argc,argv,"n:p:s:m:e:g:r:")) != -1)
                switch(in)
                {
                    case 'n': N_string = optarg; N = atoi(N_string.c_str());cout << "n " <<flush; break;
                    case 'p': Lp_string = optarg; P = std::pow(10.0,-atof(optarg)); cout << "p" << flush;  break;
  ...
  std::string dir = (std::string)SAR_STATE_DIR + "/nasshear/"+i_to_string(DIM)+"d/";
	//string dir = (std::string)SAR_STATE_DIR + (std::string)"sshear/";
	std::string datafilename = dir+i_to_string(DIM)+"dshear_N="+N_string+"_Lp="+Lp_string+"_dγ="+d_to_string(strain_step)+"_rP="+rP_string+"_seed="+seed_string+"/data.nc";
	std::string statefilename = dir+ i_to_string(DIM) + "dshear_N="+N_string+"_Lp="+Lp_string+"_dγ="+d_to_string(strain_step)+"_rP="+rP_string+"_seed="+seed_string+"/state.nc";
  ...
  NcFile::FileMode mode; 
	...
	if (file_exists(statefilename)) {
		...
		if (depth < 0) { depth = 0;} //accounts for recording period > 1

		if (depth > 0) { //infer tempcount
  ...
  cout << dir+ i_to_string(DIM) + "dshear_N="+N_string+"_Lp="+Lp_string+"_dγ="+d_to_string(strain_step)+"_rP="+rP_string+"_seed="+seed_string<< "\n";

	if (existing_M == 0) {
		s.RandomizePositions(seed);
		s.SetRadiiBi();
		s.SetPackingFraction(guess_phi);
