...
#include <unistd.h>
#include <netcdfcpp.h>
#include "Protocols/MemoryBoxyStrain.h"
...
int main(int argc, char *argv[])
{
  ...
  int steps = 0;
		...
		dbl guess_phi = 0.95;
	int in;
        while((in = getopt(argc,argv,"n:p:s:m:e:g:r:x:")) != -1)
                switch(in)
                {
                    case 'n': N_string = optarg; N = atoi(N_string.c_str());cout << "n " <<flush; break;
                    case 'p': Lp_string = optarg; P = std::pow(10.0,-atof(Lp_string.c_str())); cout << "p" << flush;  break;
  ...
  Protocols::DoubleMemoryBoxyStrain<DIM> mem(&s,&miner,strain_tensor);
